...
# Codex CLI runner for jut skill eval.
# Called by jut-integration.ts provider with JUT_EVAL_* env vars.

CODEX_BIN="${JUT_EVAL_CODEX_BIN:-${JUT_EVAL_RUNNER_BIN:-codex}}"
PROMPT="${JUT_EVAL_PROMPT:-}"
MODEL="${JUT_EVAL_MODEL:-}"
APPEND_SYSTEM_PROMPT="${JUT_EVAL_APPEND_SYSTEM_PROMPT:-}"
MIN_CODEX_VERSION="${JUT_EVAL_MIN_CODEX_VERSION:-${JUT_EVAL_MIN_RUNNER_VERSION:-0.99.0}}"
...
# Codex doesn't have --append-system-prompt, prepend to prompt
full_prompt="$PROMPT"
