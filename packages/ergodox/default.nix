...
, gnumake
, git
, avrPkgs ? null  # Cross-compilation toolchain, must be passed from flake.nix
}:

assert avrPkgs != null || throw ''
  ergodox-firmware requires avrPkgs (AVR cross-compilation toolchain).
  Use: nix build .#ergodox-firmware
  This package is configured in flake.nix with proper cross-compilation.
'';
