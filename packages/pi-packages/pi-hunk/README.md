...
- `hunk_diff` — open a Hunk diff review in a new Herdr pane or tab.
...
`hunk_diff` creates its target directly through Herdr because the Hunk UI is interactive and should not block Pi's tool call. Workspace bootstrap and persistent development layouts are owned separately by Workspace Manager.
