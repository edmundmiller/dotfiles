...
export const pull = tool({
  ...
  args: {
    ...
    flavor: tool.schema
      .string()
      .optional()
      .describe("Use a specific config flavor"),
    quiet: tool.schema
      .boolean()
      .optional()
...
export const add_target = tool({
  ...
  args: {
    name: tool.schema
      .string()
      .describe("Target name (e.g., 'github_neworg_issues')"),
    service: tool.schema
      .string()
    ...
    query: tool.schema
      .string()
      .optional()
      .describe("GitHub/GitLab query string"),
    orgs: tool.schema
...
function generateTargetConfig(args: {
  ...
  if (args.service === "github") {
    ...
    if (args.is_pr) {
      lines.push(
        `description_template = "PR {{githubnamespace}}/{{githubrepo}}: {{githubtitle}}"`
      )
    } else {
      lines.push(
        `description_template = "{{githubnamespace}}/{{githubrepo}}: {{githubtitle}}"`
      )
    }
