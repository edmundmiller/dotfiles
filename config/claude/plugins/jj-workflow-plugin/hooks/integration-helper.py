...
def check_current_state():
    ...
    has_working_changes = "Working copy changes:" in status_output and any(
        line.strip().startswith(("M ", "A ", "D "))
...
def main():
    ...
    has_description, is_empty, has_working_changes, changed_files = (
        check_current_state()
    )
    ...
    if needs_validation:
        ...
        and changed_files >= SIGNIFICANT_CHANGE_THRESHOLD
    ):
        messages.append("💡 **Substantial uncommitted work detected!**")
        messages.append("")
        messages.append(
            f"You have {changed_files} changed files that haven't been described yet."
        )
        messages.append("")
        ...
        messages.append("")
        messages.append(
            "This ensures your work is properly tracked before the next session."
        )
