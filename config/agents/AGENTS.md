...
- **Thin core**: `config/agents/core.md` → OMP and Codex startup instructions;
  maximum 220 words
- **Legacy rules**: `config/agents/rules/` → Concatenated into Claude and Pi
  instructions; symlinked to OpenCode rules during their migration

## Key Facts

- Skills use skills.sh format (YAML frontmatter + markdown)
- OMP uses the thin core plus conditional rules under `config/omp/rules/`.
- Codex uses the thin core plus task-selected skills, nested `AGENTS.md`, and
  deterministic hooks. It has no OMP-style dormant behavioral rule layer.
- Legacy rules are numbered for deterministic Claude/Pi concatenation. Do not
  grow that compatibility bundle; put procedures in skills or scoped docs.
