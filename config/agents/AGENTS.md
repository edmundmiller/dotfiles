...
Standing unslop writing rules live in `skills/catalog/unslop/`; do not copy
them into `core.md`.
...
- Claude does not discover that shared directory. Its module links only
  `test-quality`, `github-cli-media`, `lore`, and `pe-verify` into
  `~/.claude/skills`, preserving one canonical copy for OMP's name-based discovery.
