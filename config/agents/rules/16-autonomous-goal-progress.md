...
- For delegated work, the primary must wait for each worker and continue `CONTINUE` or `PARTIAL` results until the active outcome is landed or genuinely blocked. Require each worker to return `status`, `changed_paths`, `verification`, `landing_state`, and `next_action`.
