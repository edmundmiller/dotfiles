...
return {
      ...
      vim.api.nvim_create_autocmd("FileType", {
        ...
        callback = function()
          ...
          vim.keymap.set("n", "<leader>ts", "",
            vim.tbl_extend("force", buf_opts, { desc = "Sort tasks" }))
  ...
  -- Ensure treesitter has todotxt parser if available
  {
    "nvim-treesitter/nvim-treesitter",
    optional = true,
    opts = function(_, opts)
      -- Only add todotxt to ensure_installed if the parser exists
      local parser_config = require("nvim-treesitter.parsers").get_parser_configs()
      if parser_config.todotxt then
        opts.ensure_installed = opts.ensure_installed or {}
        vim.list_extend(opts.ensure_installed, { "todotxt" })
      end
      return opts
    end,
  },
