...
Run the appropriate semantic diff (`sem diff`, fallback `git diff`) to identify which files changed and how many entities/lines are involved. This context helps you generate a better-calibrated meta prompt.
...
1. Identify all changed files via `sem diff` (fallback `git diff`), then read every changed file in full — not just the diff hunks. For each changed file, also read the files it imports from and key files that depend on it, to understand integration points and downstream effects.
