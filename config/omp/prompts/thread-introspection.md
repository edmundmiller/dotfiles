...
- Give each worker only its shard’s manifest entries. Require the worker’s `summary` field itself to contain these headings: `Sessions inspected`, `Repeated failures or preferences`, `Evidence locations`, and `Smallest durable prevention surface`. Set the `summary` schema’s `maxLength` to 4300 so results remain inline; compress findings rather than omitting assigned paths. Under `Sessions inspected`, list every assigned path verbatim exactly once; do not rely on a separate files field. Workers never edit files.
...
- Treat parent, fork, advisor, and compaction replays as dependent evidence; count recurrence only across independent user-authored sessions.
...
3. Update an existing skill or prompt only when a change would prevent recurrence.
...
Changes to startup context require an explicit human decision and a runtime-specific
evaluation. Report a supported proposal instead.
Do not edit runtime symlinks under `~/.omp`, `~/.pi`, or `~/.claude`.
Do not edit secrets, lock files, generated files, or unrelated source.
Do not add broad generic advice. Extend a skill only when a reusable procedure
would have changed behavior in an observed thread.
Do not create a new skill unless no existing skill or prompt can naturally hold the lesson.
