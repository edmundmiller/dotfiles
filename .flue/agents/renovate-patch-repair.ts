...
Read the repository and target AGENTS.md instructions first. This snapshot has no \`.git\` directory. Reproduce the failure with \`nix develop --command pkg-check <target>\`. Preserve each patch's intent, drop patches already present upstream, and regenerate only conflicting patches from the updated pinned source. Run the same pkg-check until it passes.
