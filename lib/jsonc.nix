    ...
    let
      ...
      removeTrailingCommas =
          ...
          step =
            ...
            let
              ...
              outLine =
                if isBlank then
                  line
                else if acc.closingNext && lib.hasSuffix "," (trimEnd line) then
                  clean
                else
                  line;
            in
            {
              closingNext =
                if isBlank then acc.closingNext else lib.hasPrefix "]" trimmed || lib.hasPrefix "}" trimmed;
        ...
        (builtins.foldl' step {
          closingNext = false;
          lines = [ ];
        } (lib.reverseList stripped)).lines;
    in
