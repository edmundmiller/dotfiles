...
- **`tnote` wrapper is mainline-pinned via worktree**: `~/.local/bin/tnote` is managed to prefer `~/src/personal/tn-monorepo-main`, a clean detached worktree at `origin/main`, while leaving the user's dirty `~/src/personal/tn-monorepo` checkout alone as a fallback. If `tnote` seems stale, inspect both the wrapper and the worktree before touching the active repo.
