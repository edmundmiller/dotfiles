...
verification: hey check and hey re.
...
- **No passwordless sudo** — `hey re` requires a password. Use `tmux-bash` to
  launch the rebuild so the user can enter their password in the tmux pane:
  ```
  tmux-bash name=rebuild command="cd ~/.config/dotfiles && hey re"
  ```
