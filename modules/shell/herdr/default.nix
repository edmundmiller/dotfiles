    ...
    home-manager.users.${config.user.name} =
      { lib, ... }:
      {
        home.activation.herdr-config-bootstrap = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
          ...
          in_worktrees = False
          saw_worktrees = False
          wrote_worktree_keys = set()
          for line in out:
              stripped = line.strip()
              if stripped.startswith("[") and stripped.endswith("]"):
                  if in_worktrees:
                      for key, value in worktree_keys.items():
                          if key not in wrote_worktree_keys:
                              result.append(f'{key} = "{value}"')
                              wrote_worktree_keys.add(key)
              ...
              if in_worktrees and "=" in stripped:
                  ...
                  if key in worktree_keys:
                      if key not in wrote_worktree_keys:
                          result.append(f'{key} = "{worktree_keys[key]}"')
                          wrote_worktree_keys.add(key)
                      continue
          ...
          if not saw_worktrees:
              if result and result[-1].strip():
                  result.append("")
              result.append("[worktrees]")
              for key, value in worktree_keys.items():
                  result.append(f'{key} = "{value}"')
