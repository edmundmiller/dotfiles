...
git-open, difftastic, git-ai (when `ai.enable`), hunk (when `hunk.enable`), sem, lazydiff, diffity, delta, git-crypt (if gnupg enabled), git-lfs, pre-commit, git-hunks, ghui (Darwin Homebrew: `kitlangton/tap/ghui`)
