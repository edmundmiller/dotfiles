...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    # Note: Wrapping home-manager.users defers ${config.user.name} evaluation
    # to avoid infinite recursion
    home-manager.users = {
      ${config.user.name} = { config, lib, osConfig ? null, ... }:
      let
        # Darwin: home-manager has its own age module (config.age.secrets)
        # NixOS: agenix secrets are at system level (osConfig.age.secrets)
        syncSecretPath =
          if isDarwin
          then config.age.secrets.taskchampion-sync.path
          else osConfig.age.secrets.taskchampion-sync.path;
      in {
