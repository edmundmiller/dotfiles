...
let
  ...
  renderedSettings = renderHermesSettings {
    timezone = config.time.timeZone or "";
...
in
{
  ...
  config = mkIf cfg.enable {
    assertions = [
      {
        assertion = !isDarwin;
        message = "modules.agents.hermes is NixOS-only for declarative Hermes Gateway/runtime setup. Install Hermes Desktop manually on macOS.";
      }
    ];
