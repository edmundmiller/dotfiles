...
belong in Nix.
`~/.omp/agent/lsp.json` is generated by the module. It replaces OMP's built-in
TypeScript language server with the Nix-pinned Effect Language Service and
points the Nextflow LSP at the Nix-built launcher, not a Claude-plugin-root
placeholder. The `permission-policy-guard` extension is
...
`test-lsp-config.sh` builds each Darwin host's generated LSP config and verifies both
Nix-managed server commands.
