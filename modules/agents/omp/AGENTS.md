...
`~/.omp/agent/mcp.json` is rendered from `config/omp/mcp.json` plus per-host
`mcpServers` and `disabledMcpServers`. Edit repo sources, not runtime symlinks.
...
against `omp config list --json`. `test-mcp-json.sh` verifies the shared MCP
policy. Legacy/renamed keys fail closed.
