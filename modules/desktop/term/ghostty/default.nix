  ...
  config = mkIf cfg.enable {
    ...
    home-manager.users.${config.user.name} = {
      ...
      home.file = {
        ...
        # macOS Ghostty prefers Application Support when this file exists.
        # Point that location at the XDG config so a stale autogenerated template
        # does not shadow the Herdr startup command. Keep this as a tiny shim
        # instead of duplicating the full config because `config-file = ui.conf`
        # entries are relative to the containing config file.
        "Library/Application Support/com.mitchellh.ghostty/config" = {
          text = ''
            config-file = ${config.user.home}/.config/ghostty/config
          '';
          force = true;
        };
      };
      ...
      xdg.configFile = {
        ...
        "ghostty/behavior.conf".text = builtins.readFile "${configDir}/ghostty/behavior.conf" + ''
          ...
          # Priority: herdr > jmux > plain tmux.
