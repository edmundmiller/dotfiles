...
Vitest Evals template for checking whether a Pi-compatible agent follows the pytest skill.
...
Copy `skill.eval.ts` into the consuming project, replace `createPytestSkillAgent()` with the real Pi-compatible agent factory, then run:
