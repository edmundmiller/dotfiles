...
Vitest Evals template for checking whether a Pi-compatible agent follows the nf-test skill.
...
Copy `skill.eval.ts` into the consuming project, replace `createNfTestSkillAgent()` with the real Pi-compatible agent factory, then run:
