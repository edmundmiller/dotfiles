...
A scene is a **saved snapshot of target entity states, applied atomically** — not a sequential script. Activating it pushes every listed entity toward its stored state at once (optionally with a transition). Use a scene to _restore a set of states_; use a script's `sequence` when you need ordered, conditional, or timed steps.
...
- UI-created scenes live in `scenes.yaml` and snapshot the _current_ state of each added entity at save time.
