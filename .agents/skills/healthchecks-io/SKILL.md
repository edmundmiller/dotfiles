...
```bash
HC_KEY="hcw_Xxfgcx40LFjb2JJlDJvDainCDIXg"
...
curl -s "$HC_API/checks/" -H "X-Api-Key: $HC_KEY" \
...
```bash
curl -s -X DELETE "$HC_API/checks/<uuid>" -H "X-Api-Key: $HC_KEY" \
...
curl -s "$HC_API/checks/<uuid>" -H "X-Api-Key: $HC_KEY" \
  | jq '{name, status, n_pings, last_ping, next_ping}'

# Recent ping log for a check
curl -s "$HC_API/checks/<uuid>/pings/" -H "X-Api-Key: $HC_KEY" \
...
```bash
curl -s -X POST "$HC_API/checks/<uuid>/pause" -H "X-Api-Key: $HC_KEY"
curl -s -X POST "$HC_API/checks/<uuid>/resume" -H "X-Api-Key: $HC_KEY"
```
