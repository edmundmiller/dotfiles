...
- Full system build, `AGENT=1 hey re`, and the final `hey skills-sync`:
...
No further work remains. Publication was authorized on 2026-08-31 after the
primary checkout was rebased onto current `origin/main`, obsolete overlapping
changes were dropped, and the surviving activation fixes passed `hey check
--worktree` and `hey re`.
...
The surviving changes ship directly on `main`; their commit history is the
